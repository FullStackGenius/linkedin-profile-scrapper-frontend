...
const apiService = {
  ...
  // Set JWT token in localStorage (no need to set in headers here)
    ...
    try {
      ...
      if (status) {
        switch (status) {
          ...
          case 401:
            message =
              responseMessage || 'Unauthorized access. Please log in again.';
            break;
          case 403:
            message =
              responseMessage || 'Forbidden. You do not have permission.';
            break;
          case 422:
